...
4. **Config `env` block** in `~/.tunacosgun/openclaw.json` (applied only if missing).
...
| `OPENCLAW_CONFIG_PATH` | Override the config file path (default `~/.tunacosgun/openclaw.json`).                                                                                                             |
