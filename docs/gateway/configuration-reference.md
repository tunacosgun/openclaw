...
Core config reference for `~/.tunacosgun/openclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/tunacosgun/openclaw.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/tunacosgun/openclaw-YYYY-MM-DD.log`.
